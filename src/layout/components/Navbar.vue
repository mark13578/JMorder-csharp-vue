<template>
  <div class="navbar" :data-theme="sideTheme" :class="appStore.device + ' nav' + settingsStore.navType">
    ...
    <div class="right-menu">
      ...
      <template v-if="appStore.device == 'desktop'">
        <zr-git title="源碼地址" class="right-menu-item" />
        <zr-doc title="文檔地址" class="right-menu-item" />
        <screenfull title="全屏" class="right-menu-item" />
      </template>
      <size-select title="佈局大小" class="right-menu-item" />
      <LangSelect title="語言設置" class="right-menu-item" />
      ...
      <div class="right-menu-item mr10" @click="handleCommand('setLayout')" hidden>
